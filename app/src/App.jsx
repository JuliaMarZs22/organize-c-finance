...
import {
  ...
  FileSpreadsheet, Loader2, Trash2, Settings, AlertCircle, CheckCircle2,
} from "lucide-react";
import {
...
function Login({onLogin}) {
  const [email,setEmail]=useState("");const[pw,setPw]=useState("");const[erro,setErro]=useState("");const[load,setLoad]=useState(false);
  ...
  const submit=async()=>{if(!email.trim())return setErro("Digite seu e-mail.");if(!pw)return setErro("Digite sua senha.");setErro("");setLoad(true);const{user,error}=await entrar(email.trim(),pw);setLoad(false);if(error||!user){if(error?.message?.includes("Invalid login"))return setErro("E-mail ou senha inválidos.");if(error?.message?.includes("Email not confirmed"))return setErro("Confirme seu e-mail antes de entrar.");return setErro(error?.message||"Não foi possível entrar.");}onLogin(user);};
    ...
    <div className="w-full" style={{maxWidth:380}}>
      ...
      <div className="rounded-3xl p-7" style={{background:C.panel,border:`1px solid ${C.border2}`}}>
        ...
        <div className="flex flex-col gap-3">
          <div style={{position:"relative"}}><Mail size={17} color={C.faint} style={{position:"absolute",left:14,top:14}}/><input value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="seu@email.com" type="email" autoComplete="email" style={field}/></div>
          <div style={{position:"relative"}}><Lock size={17} color={C.faint} style={{position:"absolute",left:14,top:14}}/><input value={pw} onChange={(e)=>setPw(e.target.value)} type="password" placeholder="Senha" autoComplete="current-password" onKeyDown={(e)=>e.key==="Enter"&&submit()} style={field}/></div>
          {erro&&<div className="flex items-center gap-2" style={{fontSize:12.5,color:C.coral}}><AlertCircle size={13}/> {erro}</div>}
          <button onClick={submit} disabled={load} className="rounded-xl py-3 mt-1 flex items-center justify-center gap-2" style={{background:load?C.goldDeep:C.gold,color:"#16213a",fontSize:14.5,fontWeight:600,cursor:load?"not-allowed":"pointer"}}>{load?<><Loader2 size={17} className="animate-spin"/> Entrando…</>:"Entrar"}</button>
        </div>
      </div>
    </div>
  </div>;
...
function Client({user,logout}) {
  const [tab,setTab]=useState("visao");const[txs,setTxs]=useState([]);const[perfil,setPerfil]=useState(null);const[despesas,setDespesas]=useState([]);const[load,setLoad]=useState(true);
  const[showToast,toastNode]=useToast();
  const recarregar=useCallback(async()=>{try{const[{txs:t},p,{despesas:d}]=await Promise.all([carregarLancamentos(),meuPerfil(),carregarDespesasFixas()]);setTxs(t);setPerfil(p);setDespesas(d);}catch(e){console.error(e);}finally{setLoad(false);}},[]);
  ...
  const calc=useMemo(()=>{
    ...
    txs.forEach((t)=>{if(mk(new Date(t.date+"T12:00:00"))===cur){if(t.type==="entrada")entradasMes+=t.amount;else{saidasMes+=t.amount;if(t.category==="Pró-labore")proLabore+=t.amount;else if(t.category==="Investimento")investido+=t.amount;catMap[t.category]=(catMap[t.category]||0)+t.amount;}}});
    ...
    const podeGastar=caixa+prox.entrada-totalFixo-reserva;const coberto=caixa+prox.entrada>=totalFixo+reserva;
  ...
  return <div className="mx-auto px-4 py-5" style={{maxWidth:1060}}>
    ...
    {tab==="visao"&&<Visao calc={calc} perfil={perfil} user={user} onPlanilha={recarregar} txs={txs} despesas={despesas} showToast={showToast}/>}
...
function DriveCard({perfil,showToast}) {
  const[load,setLoad]=useState(false);
  const conectar=async()=>{setLoad(true);try{const{data:{session}}=await supabase.auth.getSession();if(!session)throw new Error("Sessão expirada.");await conectarDrive(session.access_token);showToast("Planilha criada no Google Drive! ✅");}catch(e){showToast(e.message||"Erro ao conectar Drive.","error");}finally{setLoad(false);}};
  return <Panel style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
    ...
    <div style={{flex:1,minWidth:200}}><div style={{fontSize:14,fontWeight:500}}>Sua planilha no Google Drive</div><div style={{fontSize:12.5,color:C.muted}}>{perfil?.planilha_url?"Conectada — sua cópia viva, no seu Drive.":"Conecte o Google e a planilha nasce no seu próprio Drive."}</div></div>
    {perfil?.planilha_url?<a href={perfil.planilha_url} target="_blank" rel="noopener noreferrer" className="rounded-lg px-4 py-2" style={{background:C.panel2,border:`1px solid ${C.border2}`,color:C.text,fontSize:13,textDecoration:"none"}}>Abrir planilha</a>:<button onClick={conectar} disabled={load} className="rounded-lg px-4 py-2 flex items-center gap-2" style={{background:C.gold,color:"#16213a",fontSize:13,fontWeight:600,cursor:load?"not-allowed":"pointer"}}>{load?<Loader2 size={15} className="animate-spin"/>:"Conectar Google Drive"}</button>}
  </Panel>;
}

function Visao({calc,perfil,user,onPlanilha,txs,despesas,showToast}) {
  return <div className="flex flex-col gap-4">
    ...
    <DriveCard perfil={perfil} showToast={showToast}/>
    <Panel>
      ...
      {calc.cats.length===0?<div style={{fontSize:13,color:C.faint,padding:16}}>Ainda sem lançamentos neste mês.</div>:<div className="flex items-center gap-4 flex-wrap"><div style={{width:150,height:150,flexShrink:0}}><ResponsiveContainer><PieChart><Pie data={calc.cats} dataKey="value" nameKey="name" innerRadius={45} outerRadius={72} paddingAngle={2} stroke="none">{calc.cats.map((c)=><Cell key={c.name} fill={colorFor(c.name)}/>)}</Pie><Tooltip formatter={(v)=>[brl(v),""]} contentStyle={{background:C.panel2,border:`1px solid ${C.border2}`,borderRadius:10,color:C.text,fontSize:12}}/></PieChart></ResponsiveContainer></div><div className="flex-1 flex flex-col gap-1.5" style={{minWidth:170}}>{calc.cats.map((c)=><div key={c.name} className="flex items-center justify-between" style={{fontSize:13}}><div className="flex items-center gap-2"><span style={{width:9,height:9,borderRadius:3,background:colorFor(c.name),display:"inline-block"}}/>{c.name}</div><div style={{color:C.muted}}>{brl(c.value)}</div></div>)}</div></div>}
    </Panel>
    {despesas.length>0&&<Panel><div style={{fontSize:14,fontWeight:500,marginBottom:10}}>Despesas fixas mensais</div>{despesas.map((d,i)=><div key={d.id} className="flex items-center justify-between py-1.5" style={{fontSize:13,borderTop:i?`1px solid ${C.border}`:"none"}}><span style={{color:C.muted}}>{d.nome}</span><span style={{color:C.coral}}>{brl(d.valor)}</span></div>)}<div className="flex items-center justify-between pt-2 mt-1" style={{fontSize:13,fontWeight:600,borderTop:`1px solid ${C.border2}`}}><span>Total fixo</span><span style={{color:C.coral}}>{brl(calc.fixa)}</span></div></Panel>}
  </div>;
}
...
function DespesasFixas({despesas,onUpdate,showToast}) {
  ...
  return <div className="grid gap-4" style={{gridTemplateColumns:"minmax(0,520px)",justifyContent:"center"}}>
    <Panel>
      ...
      <p style={{fontSize:12.5,color:C.muted,marginBottom:16}}>Aluguel, internet, assinaturas — tudo que sai todo mês.</p>
  ...
  return <div className="flex flex-col gap-4">
    <Panel style={{background:calc.coberto?`linear-gradient(160deg,#0c241d,${C.panel})`:`linear-gradient(160deg,#2c1d1a,${C.panel})`,borderColor:(calc.coberto?C.emer:C.coral)+"55"}}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1"><div style={{fontSize:15,fontWeight:600,color:calc.coberto?C.emer:C.coral}}>{calc.coberto?"Próximo mês está coberto":"Atenção: próximo mês descoberto"}</div><p style={{fontSize:13,color:C.muted,lineHeight:1.5,marginTop:4}}>Em {monthLabel(calc.prox.key)} entram <b style={{color:C.text}}>{brl(calc.prox.entrada)}</b> previstos. Com caixa de <b style={{color:C.text}}>{brl(calc.caixa)}</b>, você pode gastar até <b style={{color:C.gold}}>{brl(Math.max(0,calc.podeGastar))}</b> mantendo a reserva.</p></div>
      </div>
    </Panel>
      ...
      <Panel>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>{list.map((c)=><tr key={c.id} style={{borderBottom:`1px solid ${C.border}`}}><td style={{padding:12}}><div className="flex items-center gap-2.5"><div className="flex items-center justify-center rounded-full" style={{width:30,height:30,background:C.panel2,border:`1px solid ${C.border}`,fontSize:12,color:C.gold,fontWeight:600,flexShrink:0}}>{(c.nome||"?")[0].toUpperCase()}</div><div><div style={{fontWeight:500}}>{c.nome||"—"}</div>{c.email&&<div style={{fontSize:11,color:C.faint}}>{c.email}</div>}</div></div></td><td style={{padding:12,color:C.muted}}>{c.telefone||"—"}</td><td style={{padding:12,color:C.muted,textTransform:"capitalize"}}>{c.plano||"—"}</td><td style={{padding:12}}><Badge color={cor(c.status)}>{lbl(c.status)}</Badge></td><td style={{padding:12,color:C.muted}}>{c.criado_em?new Date(c.criado_em).toLocaleDateString("pt-BR",{day:"2-digit",month:"short",year:"numeric"}):"—"}</td></tr>)}</tbody>
          </table>
